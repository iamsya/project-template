...
def check_system_admin(user_id: str, db: Session) -> None:
    ...
    if PermissionGroup.ROLE_SYSTEM_ADMIN not in roles:
        logger.warning(
            f"시스템 관리자 권한 없음: user_id={user_id}, roles={roles}"
        )
        raise HTTPException(
...
def check_admin(user_id: str, db: Session) -> None:
    ...
    if not any(role in roles for role in allowed_roles):
        logger.warning(
            f"관리자 권한 없음: user_id={user_id}, roles={roles}"
        )
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="시스템 관리자 또는 통합 관리자 권한이 필요합니다."
        )


def check_any_role_dependency(
    request: Request,
    user_id: Optional[str] = Query(None, description="사용자 ID (테스트용. request.state.user_id가 있으면 우선 사용)"),
    db: Session = Depends(get_db),
) -> None:
    """
...
def check_any_role(user_id: str, db: Session) -> None:
    """
    모든 역할 허용 (일반 사용자 제외)
    - 시스템 관리자, 통합 관리자, 공정 관리자 중 하나라도 있으면 허용
    ...
    # 모든 공정 접근 가능 (system_admin 또는 integrated_admin)이면 허용
...
def check_process_access_by_id(
    process_id: str, user_id: str, db: Session
) -> None:
    """
    특정 공정에 대한 접근 권한 체크
    ...
    # None이면 모든 공정 접근 가능 (system_admin 또는 integrated_admin)
