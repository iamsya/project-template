...
class RoleMaster(Base):
    ...
    role_id = Column(
        ...
        String(50),
        primary_key=True,
        comment=("Role ID (PK) - " "system_admin, process_admin, process_manager"),
    )
    ...
    description = Column("DESCRIPTION", Text, nullable=True, comment="Role 설명")
    ...
    create_user = Column("CREATE_USER", String(50), nullable=False, comment="생성자")
    update_dt = Column(
        "UPDATE_DT", DateTime, nullable=True, onupdate=func.now(), comment="수정 일시"
    )
    update_user = Column("UPDATE_USER", String(50), nullable=True, comment="수정자")

    # 인덱스 정의
    __table_args__ = (
        # is_active + display_order 복합 인덱스 (활성 Role 조회 및 정렬 최적화)
        Index("idx_role_master_active_order", "IS_ACTIVE", "DISPLAY_ORDER"),
    )

    def __repr__(self):
        return (
            f"<RoleMaster(role_id='{self.role_id}', " f"role_name='{self.role_name}')>"
        )
...
class PermissionGroup(Base):
    ...
    # Primary Key
    group_id = Column(
        "GROUP_ID", String(50), primary_key=True, comment="권한 그룹 ID (PK)"
    )
    ...
    description = Column("DESCRIPTION", Text, nullable=True, comment="권한 그룹 설명")
    ...
    ROLE_PROCESS_ADMIN = "process_admin"
    ...
    deleted_at = Column("DELETED_AT", DateTime, nullable=True, comment="삭제 일시")
    deleted_by = Column("DELETED_BY", String(50), nullable=True, comment="삭제자")

    # 인덱스 정의
    __table_args__ = (
        # is_active + is_deleted 복합 인덱스 (활성 그룹 조회 최적화)
        Index("idx_permission_group_active_deleted", "IS_ACTIVE", "IS_DELETED"),
        # role_id 단일 인덱스 (role별 그룹 조회 최적화)
        Index("idx_group_role", "ROLE_ID"),
    )
...
class GroupProcessPermission(Base):
    ...
    permission_id = Column(
        "PERMISSION_ID", String(50), primary_key=True, comment="권한 ID (PK)"
    )
    ...
    __table_args__ = (
        ...
        Index("idx_group_process_unique", "GROUP_ID", "PROCESS_ID", unique=True),
        # group_id + is_active 복합 인덱스 (그룹별 활성 권한 조회 최적화)
        Index("idx_group_process_group_active", "GROUP_ID", "IS_ACTIVE"),
        # process_id + is_active 복합 인덱스 (공정별 활성 권한 조회 최적화)
        Index("idx_group_process_process_active", "PROCESS_ID", "IS_ACTIVE"),
    )
...
class UserGroupMapping(Base):
    ...
    # Primary Key
    mapping_id = Column(
        "MAPPING_ID", String(50), primary_key=True, comment="매핑 ID (PK)"
    )
    ...
    __table_args__ = (
        ...
        Index("idx_user_group_user_active", "USER_ID", "IS_ACTIVE"),
        # group_id + is_active 복합 인덱스 (그룹별 활성 매핑 조회 최적화)
        Index("idx_user_group_group_active", "GROUP_ID", "IS_ACTIVE"),
    )
