...
"""
Document Models - ai_backend 전용
ai_backend의 Base를 사용하여 Foreign Key 제약조건이 작동하도록 정의
"""
import uuid
...
class Document(Base):
    ...
    # Program 관련 필드 (같은 Base 사용하므로 Foreign Key 작동)
    program_id = Column(
        'PROGRAM_ID', 
        String(50), 
    ...
    knowledge_reference_id = Column(
        'KNOWLEDGE_REFERENCE_ID', 
        String(50), 
        ForeignKey('KNOWLEDGE_REFERENCES.REFERENCE_ID'), 
        nullable=True, 
        index=True
    )
...
class ProcessingJob(Base):
    ...
    # Program 참조 (같은 Base 사용하므로 Foreign Key 작동)
    program_id = Column(
        "PROGRAM_ID", 
        String(50), 
        ForeignKey("PROGRAMS.PROGRAM_ID"), 
