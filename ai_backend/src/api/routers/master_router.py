...
@router.get(
    ...
    **권한 기반 필터링:**
    - `user_id`: 사용자 ID (테스트용, request.state.user_id가 있으면 우선 사용)
    ...
    **프론트엔드 사용 흐름:**
    1. API 호출: `GET /v1/masters` (SSO 사용 시) 또는 `GET /v1/masters?user_id=user001` (테스트용)
    ...
    **사용 예시:**
    ```
    GET /v1/masters  # SSO 사용 시 (request.state.user_id 자동 사용)
    GET /v1/masters?user_id=user001  # 테스트용
    ```
    """,
)
def get_masters_for_dropdown(
    ...
    try:
        ...
        masters = master_crud.get_masters_for_mapping_dropdown(accessible_process_ids)
...
@router.get(
    ...
    **사용 예시:**
    - 활성 공정만 조회: `GET /v1/masters/processes` (SSO 사용 시) 또는 `GET /v1/masters/processes?user_id=user001` (테스트용)
...
def get_processes(
    ...
    try:
        ...
        return ProcessListResponse(items=items, total_count=len(items))
    except Exception as e:
