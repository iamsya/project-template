...
from src.core.permissions import check_system_admin_dependency
...
def create_user(
    ...
    user = user_service.create_user(
        user_id=request.user_id, employee_id=request.employee_id, name=request.name
    )
...
def get_user_by_employee_id(
    employee_id: str, user_service: UserService = Depends(get_user_service)
):
    """사번으로 사용자 정보를 조회합니다."""
    # Service Layer에서 전파된 HandledException을 그대로 전파
    # Global Exception Handler가 자동으로 처리
...
def update_user(
    ...
    user = user_service.update_user(
        user_id=user_id, name=request.name, employee_id=request.employee_id
    )
...
def deactivate_user(
    ...
    return UserStatusResponse(
        user_id=user_id, is_active=False, message="사용자가 비활성화되었습니다."
    )


@router.patch("/users/{user_id}/activate", response_model=UserStatusResponse)
def activate_user(user_id: str, user_service: UserService = Depends(get_user_service)):
    """사용자를 활성화합니다."""
    # Service Layer에서 전파된 HandledException을 그대로 전파
    # Global Exception Handler가 자동으로 처리
    ...
    return UserStatusResponse(
        user_id=user_id, is_active=True, message="사용자가 활성화되었습니다."
    )
